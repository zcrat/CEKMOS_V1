...
class ConceptosPresupuestosController extends Controller
{
    ...
    public function categoriasSatOptions(Request $request): JsonResponse
    {
    ...
    public function unidadesSatOptions(Request $request): JsonResponse
    {
        $query = trim((string) $request->input('query', ''));
    ...
    public function vehiculosOptions(Request $request): JsonResponse
    {
        $validated = $request->validate([
        ...
        $options = VehiculosConceptosDisponibles::query()
            ...
            ->limit(50)
            ->get()
            ->map(function (VehiculosConceptosDisponibles $item) {
                return [
    ...
    public function categoriasOptions(Request $request): JsonResponse
    {
        $query = trim((string) $request->input('query', ''));
        $options = Tipos::query()
    ...
    {
        $costo->load('concepto_presupuesto');
        $concepto = $costo->concepto_presupuesto;
        ...
        return response()->json([
            ...
            'modulo_id' => $concepto?->modulo_orden_servicio_id,
            'categoria_sat_id' => $concepto?->categoria_sat_id,
            'unidad_sat_id' => $concepto?->unidad_sat_id,
            'vehiculo_id' => $costo->vehiculo_concepto_id,
    ...
    public function update(
        Request $request,
        CostosConceptosPresupuestos $costo
    ): JsonResponse {
        $validated = $this->validateConcepto($request);
