...
use App\Services\FlujoSeguimientoOrdenServicio;
...
class RecepcionVehicularController extends Controller
{
    ...
    public function Read(Request $request)
    {
        ...
        $query = OrdenesServicio::query()
            ...
            ->withCount([
                'subcontratos',
                'subcontratos as subcontratos_activos_count' => fn ($query) => $query->whereNull('fecha_fin'),
            ])
        ...
        $items = $query->map(function ($item) {
            $estatus = $item->last_seguimiento?->estatus?->descripcion;
            ...
            return [
                ...
                'taller' => optional($item->taller)->descripcion ?? 'Sin taller',
                'modulo_id' => $item->modulo_orden_id,
                'modulo' => optional($item->modulo_ordenes_servicio)->descripcion ?? 'Sin módulo',
                'user_asignado' => $item->user_asignado,
                'usuario_asignado' => $item->usuario_asignado
                    ? str_replace('-', ' ', $item->usuario_asignado->name)
                    : 'Sin asignar',
                'subcontratos_count' => $item->subcontratos_count,
                'tiene_subcontrato_activo' => $item->subcontratos_activos_count > 0,
    ...
    public function actualizarSeguimiento(
        ...
        $seguimiento = DB::transaction(function () use (
            ...
            OrdenesServicio::query()
                ->whereKey($ordenServicio->id)
                ->lockForUpdate()
                ->firstOrFail();

            $ultimoSeguimiento = SeguimientoUnidades::query()
                ->with('estatus')
                ->where('orden_servicio_id', $ordenServicio->id)
                ->latest('id')
                ->lockForUpdate()
                ->first();
            $estatusActual = $ultimoSeguimiento?->estatus?->descripcion;
            $accion = FlujoSeguimientoOrdenServicio::accion(
                $estatusActual,
                $validated['accion']
            );
            ...
            if (! $accion) {
                throw ValidationException::withMessages([
                    'accion' => 'La acción ya no está disponible para el estatus actual.',
                ]);
            }

            abort_unless(
                $request->user()->can($accion['permiso']),
                403,
            ...
            $estatusDestino = Estatus::query()
                ...
                ->where('descripcion', $accion['destino'])
                ->firstOrFail();

            return SeguimientoUnidades::create([
                'orden_servicio_id' => $ordenServicio->id,
                'estatus_id' => $estatusDestino->id,
            ]);
        });

        $seguimiento->load('estatus');

        return response()->json([
            'message' => 'Estatus actualizado correctamente.',
            'estatus' => $seguimiento->estatus->descripcion,
        ]);
    }

    public function eliminarUltimoSeguimiento(
        Request $request,
        OrdenesServicio $ordenServicio
    ) {
        abort_unless(
        ...
        $estatusAnterior = DB::transaction(function () use ($ordenServicio) {
            OrdenesServicio::query()
                ->whereKey($ordenServicio->id)
                ->lockForUpdate()
                ->firstOrFail();
            ...
            if (! $ultimoSeguimiento) {
                throw ValidationException::withMessages([
                    'seguimiento' => 'La orden no tiene un seguimiento que se pueda eliminar.',
                ]);
            }

            $ultimoSeguimiento->delete();

            return SeguimientoUnidades::query()
                ->with('estatus')
                ->where('orden_servicio_id', $ordenServicio->id)
                ->latest('id')
                ->first()
        ...
        return response()->json([
            'message' => 'El último estatus de seguimiento fue eliminado.',
            'estatus' => $estatusAnterior ?? 'Sin diagnóstico',
        ]);
    }
    ...
    public function Create(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
            $ubicacion = Ubicaciones::FirstOrCreate(['nombre' => strtolower(trim($request->ubicacion)),
            ]);
            $funtions = (new FunctionsOrdenServicio);
            $orden = $funtions->GetClave($request->modulo_orden_id);
            $seguimiento = $request->orden_seguimiento;
            $opcional = $request->orden_opcional;
            ...
            $administrador = UsuariosTaller::FirstOrCreate(['nombre' => strtolower(trim($request->administrador)),
                'tipo_id' => 1,
            ]);
            $jefetaller = UsuariosTaller::FirstOrCreate(['nombre' => strtolower(trim($request->jefe)),
                'tipo_id' => 2,
            ]);
            $tecnico = UsuariosTaller::FirstOrCreate(['nombre' => strtolower(trim($request->tecnico)),
                'tipo_id' => 4,
            ]);
            $trabajador = UsuariosTaller::FirstOrCreate(['nombre' => strtolower(trim($request->trabajador)),
                'tipo_id' => 3,
            ]);
            ...
            $presupuesto = Presupuestos::create([
                ...
                'folio' => $$funtions->GetFolio($ordenservicio->id, $orden, $request->tipo_presupuesto_id),
                'vigencia' => Carbon::now(),
                'factura_id' => null,
    ...
    public function Update(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'condiciones_interiores' => ['required', 'array'],
            'condiciones_exteriores' => ['required', 'array'],
            'imagenes_evidencia' => ['nullable', 'array', 'min:1'],
            'imagenes_evidencia.*' => ['required', 'file', 'image', 'max:20048'],
        ...
        try {
            ...
            $ordenservicio->update([
                ...
                'orden_opcional' => $request->orden_opcional,
                'vehiculo_id' => $request->vehiculo_id,
                'user_id' => $request->user()->id,
                'empresa_id' => $request->empresa_id,
                'cliente_id' => $request->cliente_id,
            ...
            ResponsablesOrdenServicio::updateOrCreate(['orden_servicio_id' => $ordenservicio->id], [
                'administrador_transporte_id' => $administrador->id,
                'jefe_de_proceso_id' => $jefetaller->id,
                'trabajador_id' => $trabajador->id,
                'tecnico_id' => $tecnico->id,
            ...
            if ($recepcionVehicular->cambiar_archivos) {
                ...
                if ($request->hasFile('imagenes_evidencia')) {
                    foreach ($request->file('imagenes_evidencia') as $i => $file) {
                        $ext = $file->getClientOriginalExtension();
                        $file->storeAs(
                            $pathevidencia->folder ?? 'desconocidos',
