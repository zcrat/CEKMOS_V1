...
class PdfController extends Controller
{
    ...
    public function ValeAlmacen(int $id)
    {
        $vale = ValesAlmacen::findOrFail($id);
        $vale->load('orden_servicio');

        abort_unless(
            AlcanceRecepcionesVehiculares::puedeAccederOrden(request()->user(), $vale->orden_servicio),
            403
        );
        $valeAlmacenPdf = app(ValeAlmacenPdfService::class);

        if (! $valeAlmacenPdf->exists($vale)) {
            $valeAlmacenPdf->generate($vale);
        }

        return response()->file(
            $valeAlmacenPdf->absolutePath($vale),
            [
                'Content-Type' => 'application/pdf',
                'Content-Disposition' => 'inline; filename="'.$valeAlmacenPdf->fileName($vale).'"',
    ...
    public function RecepcionVehicular($id)
    {
        ...
        if ($emisor) {
            $bloques = [];
            if (! empty($emisor->calle)) {
                $bloques[] = $emisor->calle;
            }
            // Colonia sin el prefijo "COL.", seguido de punto, y CP
            $col_cp = '';
            if (! empty($emisor->colonia)) {
                $col_cp .= $emisor->colonia.'. ';
            }
            if (! empty($emisor->cp)) {
                $col_cp .= 'C.P. '.$emisor->cp;
            }
            $col_cp = trim($col_cp);
            if (! empty($col_cp)) {
                $bloques[] = $col_cp;
            ...
            if (! empty($emisor->ciudad)) {
                $bloques[] = $emisor->ciudad;
            }
            if (! empty($emisor->estado)) {
                $bloques[] = $emisor->estado;
            }
            if (! empty($emisor->telefono)) {
                $bloques[] = 'TEL '.$emisor->telefono;
            }
    ...
    public function InspeccionVehicular($id)
    {
        $ordenservicio = OrdenesServicio::with([
            'empresa',
            'entrada',
